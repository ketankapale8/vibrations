...
export default function PracticeQuestions1() {
    ...
    const [color , setColor] = useState(COLORS.lightpurple)
    const [text , setText] = useState('Next')
    const currentQuestionNo = questions?.map((item)=> item?.QuestionNo == currentQue);
            ...
            {currentQuestionNo.map(item=>{
                ...
                {item?.AnswerArray?.map(ans =>{
                    return (
                        ...
                        <TouchableOpacity onPress={()=>checkCorrectAnswer(ans.Answer1 ,ans.CorrectAns)}>
                            <View style={{ width : SIZES.width -220, margin: SIZES.padding , padding: SIZES.padding , backgroundColor :  color , borderRadius: 20 , justifyContent:"center" , display:"flex" , height : SIZES.height * 0.1 }}>
                                <Text style={{color : COLORS.black , fontSize : SIZES.body2}}>{ans.Answer1}</Text>
                            </View>
                        </TouchableOpacity>
                        <TouchableOpacity onPress={()=>checkCorrectAnswer(ans.Answer2 ,ans.CorrectAns)} >
                            <View style={{ width : SIZES.width -220, margin: SIZES.padding , padding: SIZES.padding , backgroundColor : color , borderRadius: 20 , justifyContent:"center" , display:"flex" , height : SIZES.height * 0.1 }}>
    ...
    const checkCorrectAnswer = (ans,correctAns) =>{
        if(ans == correctAns){
            return Toast.show({
                type: 'success',
                text1: `${ans} is a correct answer`,
                
              });
        }else if(ans !==correctAns){
            return Toast.show({
                type: 'error',
                text1: `${ans} is an incorrect answer`,
